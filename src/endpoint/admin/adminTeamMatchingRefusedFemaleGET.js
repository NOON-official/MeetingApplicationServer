...
// 매칭 거절당한 여자팀 정보 전체 조회
module.exports = async (req, res) => {
  let conn;

  try {
    conn = await pool.getConnection();
    let femaleTeam = await teamDB.getFemaleTeamByStateAndPageNum(conn, 4, 9);
    ...
    res.status(statusCode.OK).send(
      util.success(statusCode.OK, responseMessage.GET_MATCHING_REFUSED_FEMALE_TEAM_SUCCESS, {
        femaleTeam,
      }),
    );
  } catch (error) {
