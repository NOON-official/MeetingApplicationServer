...
// 매칭 상호 수락한 남자팀 정보 전체 조회
module.exports = async (req, res) => {
  let conn;

  try {
    conn = await pool.getConnection();
    let maleTeam = await teamDB.getMaleTeamByStateAndPageNum(conn, 3, 7);
    ...
    res.status(statusCode.OK).send(
      util.success(statusCode.OK, responseMessage.GET_MATCHING_BOTH_ACCEPTED_MALE_TEAM_SUCCESS, {
        maleTeam,
      }),
    );
  } catch (error) {
