...
// 여자팀 2:2 매칭 신청 정보 전체 조회 (승인 완료)
module.exports = async (req, res) => {
  let conn;

  try {
    conn = await pool.getConnection();
    let femaleTeam = await teamDB.getTeamByAdmin(conn, 2, 2, 1); // 여자, 2:2, 승인 완료
    ...
    res.status(statusCode.OK).send(
      util.success(statusCode.OK, responseMessage.GET_FEMALE_TWO_TEAM_CHECKED_APPLY_SUCCESS, {
        femaleTeam,
      }),
    );
  } catch (error) {
