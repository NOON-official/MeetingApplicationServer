...
// 매칭 수락 대기중인 남자팀 정보 전체 조회
module.exports = async (req, res) => {
  let conn;

  try {
    conn = await pool.getConnection();
    let maleTeam = await teamDB.getPendingMaleTeamByAdmin(conn, 1); // 남자
    ...
    res.status(statusCode.OK).send(
      util.success(statusCode.OK, responseMessage.GET_MATCHING_PENDING_MALE_TEAM_SUCCESS, {
        maleTeam,
      }),
    );
  } catch (error) {
