...
// 매칭 수락 대기중인 여자팀 정보 전체 조회
module.exports = async (req, res) => {
  let conn;

  try {
    conn = await pool.getConnection();
    let femaleTeam = await teamDB.getPendingFemaleTeamByAdmin(conn, 2); // 여자
    ...
    res.status(statusCode.OK).send(
      util.success(statusCode.OK, responseMessage.GET_MATCHING_PENDING_FEMALE_TEAM_SUCCESS, {
        femaleTeam,
      }),
    );
  } catch (error) {
